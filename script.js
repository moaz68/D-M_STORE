...
function addToCart(product, size, color, quantity = 1, price = null) {
    ...
    if (color && product.variants) {
        const variant = product.variants.find(v => normalizeValue(v.colorName) === normalizeValue(color));
...
function renderHomeCarousel() {
    ...
    // Render product cards into carousel
    carousel.innerHTML = products.map(product => `
        <div class="home-product-card">
            <a href="product.html?id=${product.id}" class="home-product-card__image-wrap">
                <span class="home-product-card__sale-badge">Sale</span>
                <img src="${product.mainImage}" alt="${product.name}" class="home-product-card__image" loading="lazy">
            </a>
            <div class="home-product-card__info">
                <h3 class="home-product-card__name">${product.name}</h3>
                <p class="home-product-card__price">${product.price}</p>
            </div>
            <div class="home-product-card__actions">
                <a href="product.html?id=${product.id}" class="home-product-card__btn">Choose options</a>
            </div>
        </div>
    `).join('');
    ...
    const updateDots = () => {
        ...
        const cardWidth = carousel.querySelector('.home-product-card')?.offsetWidth || 240;
        const gap = 24;
        ...
        const pages = Math.ceil(maxScroll / scrollStep) + 1;

        dotsContainer.innerHTML = Array.from({ length: Math.min(pages, 7) }, (_, i) =>
            `<button class="home-products__dot${i === 0 ? ' active' : ''}" data-page="${i}" aria-label="Go to page ${i + 1}"></button>`
        ).join('');

        // Dot click handlers
        dotsContainer.querySelectorAll('.home-products__dot').forEach(dot => {
            dot.addEventListener('click', () => {
                const page = parseInt(dot.getAttribute('data-page'));
                const scrollTo = page * scrollStep;
    ...
    const updateActiveDot = () => {
        if (!dotsContainer) return;
        const dots = dotsContainer.querySelectorAll('.home-products__dot');
        ...
        dots.forEach((dot, i) => {
            dot.classList.toggle('active', i === Math.min(currentPage, dots.length - 1));
        });
    };
    ...
    const updateActiveThumbnail = (index) => {
        ...
        thumbBtns.forEach((btn, i) => {
            if (i === activeIndex) {
                ...
                // Manually scroll the container to avoid scrollIntoView page-shift bugs on mobile
                if (thumbnailsContainer) {
                    const scrollPos = btn.offsetLeft - (thumbnailsContainer.offsetWidth / 2) + (btn.offsetWidth / 2);
